...
const Nav = () => {
  ...
  useEffect(() => {
    if (socket) {
      ...
      // Atualiza os usuários online ao receber os dados do WebSocket
      socket.on('users-online', (onlineUsers: User[]) => {
        console.log('Usuários online recebidos:', onlineUsers);
        
        // Atualiza o status de online/offline sem sobrescrever a lista de usuários
        setUsers(prevUsers => {
          return prevUsers.map(user => {
            const isOnline = onlineUsers.some(onlineUser => onlineUser.id === user.id);
            return {
              ...user,
              stats: isOnline ? 1 : 2, // 1 = online, 2 = offline
            };
          });
        });
      });
  ...
  }, [socket]); // Certifique-se de que o socket é um valor constante ou controlado
